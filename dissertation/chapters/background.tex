...
As we defined reachability and reward-based properties separately, we also define model checking rechability and reward-based properties separately, although the methods used in each type of property are similar.
...
Let $Path(s)$ be the set of all paths of some DTMC, starting in some stats $s$, and let $T$ be the set of states in the DTMC where some proposition $a$ holds, along with denoting $\rho$ and $\iota$ as state and transition reward functions respectively. Then $X_{Reach(T)} : Path(s) \Rightarrow \mathbb{R}_{\geq 0}$ is a random variable where, for any infinite path $\omega = s_{0} s_{1} s_{2} \dots$,
    ...
    \begin{equation*} 
        X_{Reach(T)}(\omega) = 
            \begin{cases}
                0 & s_0 \in T \\
                \infty & s_i \notin T \forall i \geq 0 \\
                \sum_{i=0}^{k_{T} - 1} \rho(s_i) + \iota(s_i, s_{i+1}) & \text{otherwise}
            \end{cases}
    \end{equation*}

where $k_T = min(j | s_j \in T)$, the first state in the path where $a$ holds.
...
\begin{equation*}
    ExpReach(s, T) = 
        \begin{cases}
            \infty & \text{there exists a path} \omega \in Path(s) \text{where} T \text{is never reached} \\
            0 & s \in T \\
