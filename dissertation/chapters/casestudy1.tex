...
As a final remark, adversary generation for reward-based properties is analogous to adversary generation for reachability properties, using almost identical applications of value iteration and precomputation. For this reason, precise details of constructing these adversaries are omitted.
...
\subsection{Making comparisons using strategy generation}
...
Consider the strategy which maximises the expected score of Shut the Box, which is 21 for a 6-board variant. Now consider the situation presented in Figure \ref{cs1:optimal_high_board}, where boards 2, 4 and 6 are uncovered while a 6 is rolled. The optimal strategy chooses to cover boards 4 and 2, while the high-board strategy chooses to cover board 6.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/ShutTheBox/optimal_high_board.pdf}
    \caption{The optimal strategy covers boards 4 and 2, while the high-board strategy covers board 6.}
\label{cs1:optimal_high_board}
\end{figure}

The probability of attaining the maximum possible score is higher for the high-board strategy than the optimal strategy - while the optimal strategy will require rolling a 6 next turn to attain the maximum score, the high-board strategy can also attain the maxmimum score by rolling a 4 and a 2 within two rolls, along with rolling a 6 on the first roll.

However, the expected value of the optimal strategy from this point is higher than the high-board strategy. Since the final score is 15 with probability $\frac{5}{6}$ and 21 with probability $\frac{1}{6}$, the overall expected value is $16$. Similarly, the expected value of the high-board strategy from this point is

\begin{equation*}
    (15 \times \frac{1}{2}) + (17 \times \frac{5}{36}) + (19 \times \frac{5}{36}) + (21 \times (\frac{1}{6} + \frac{1}{18}) = \frac{103}{6} \approx 17.16
\end{equation*}


\end{example}

This presents an interesting trade-off between strategies - while the optimal strategy aims to maximise the expected score of Shut the Box (by design), the high-board strategy aims to maximise the probability of attaining the maximum possible score.
...
For instance, suppose two players play a round of Shut the Box sequentially, aiming to get a higher score than the other player. The first player's aim is to get the highest score possible, but the second player's objective is merely to earn more points than the first player - the winning margin itself is totally irrelevant. If the first player scores 20 points on the 6-board variant, the second player should play the aforementioned optimal strategy, since this maximises the probability of gaining 21 points, the only possible score which can win outright. On the other hand, if the first player scores very few points, then a more consistent strategy, such as the high-board strategy, may be preferred, in order to maximise the probability of winning.Alternatively, an additional incentive could be provided to encourage players to attain a perfect score, such as a "jackpot" if Shut the Box is played as a gambling game.
...
\begin{table}[]
    ...
    \multicolumn{1}{l|}{Minimum expected score}    & 23.97423197612391  \\
    \multicolumn{1}{l|}{Low-board}  & 24.30185396277705  \\
    \multicolumn{1}{l|}{High-board} & 42.808769718128595 \\
    \multicolumn{1}{l|}{Maximum expected score}    & 42.91857934707855 
    \end{tabular}
...
While the optimal strategy for Shut the Box is somewhat complex, Section \ref{cs1:compare-strats} shows that a simple strategy is very close to optimal. This shows that Shut the Box is flawed, because the additional complexity involved in learning a more optimal strategy does not lead to a significant improvement in the overall outcome of the game. While the trade-off between risk and reward is potentially interesting, in most cases the difference is fairly small, so the impact of this trade-off is minimal.
