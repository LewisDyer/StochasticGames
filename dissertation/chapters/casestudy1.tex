...
\subsection{Comparisons with optimal strategies}
...
Consider the strategy which maximises the expected score of Shut the Box, which is 21 for a 6-board variant. Comparing this strategy with the high-board strategy shows that the strategies differ at only one position, highlighted Figure \ref{cs1:optimal_high_board}, where a 6 is rolled while only board 6 is covered. The optimal strategy chooses to cover boards 4 and 2, while the high-board strategy chooses to cover boards 5 and 1.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/ShutTheBox/optimal_high_board.pdf}
    \caption{The optimal strategy covers boards 4 and 2, while the high-board strategy covers board 5 and 1. Covering boards 1, 2 and 3 is also possible, but is not chosen by either strategy.}
\label{cs1:optimal_high_board}
\end{figure}

A modified version of the PRISM model for Shut the Box was produced, setting this position as the initial state of the model.

% this happened... difference between models?
